...
class _dataCaptureThread(object):
    ...
    def __init__(self, sensorIP, data_socket, imu_socket, filePathAndName, fileType, secsToWait, duration, firmwareType, showMessages, format_spaces, deviceType):
        ...
        self.firmwareType = firmwareType  # 0
        self.started = True
    ...
    def run_realtime_bin(self):
        ...
        try:
            ...
            # print(deviceCheck, self._deviceType)
        except:
            pass
        ...
        while True:
            ...
            if self.started:
                ...
                if selectTest[0]:
                    data_pc, addr = self.d_socket.recvfrom(1500)
                    version = int.from_bytes(data_pc[0:1], byteorder='little')
                    self.dataType = int.from_bytes(data_pc[9:10], byteorder='little')  # dataType2 = single return cartesian
                    timestamp_type = int.from_bytes(data_pc[8:9], byteorder='little')
                    timestamp1 = self.getTimestamp(data_pc[10:18], timestamp_type)
                    self.updateStatus(data_pc[4:8])
                    if self.isCapturing:
                    ...
                    if self.started:
                        ...
                        if timeSinceStart <= self.duration:
                            ...
                            if select.select([self.d_socket], [], [], 0)[0]:
                                ...
                                if self.firmwareType == 1:
                                    # https://github.com/Livox-SDK/Livox-SDK/wiki/Livox-SDK-Communication-Protocol#33-point-cloudimu-data-data_type
                                    # Cartesian Coordinate System
                                    if dataType == 0: # Cartesian/single/N=100/100,000 pps
                                        # to account for first point's timestamp being increment in the loop
                                        timestamp_sec -= 0.00001  # 1/100,000
                                    ...
                                    # Spherical Coordinate System
                                    elif dataType == 1: # Spherical/single/N=100/100,000 pps
                                        # to account for first point's timestamp being increment in the loop
                                        timestamp_sec -= 0.00001
                                    ...
                                    # Horizon/Tele-15/Avia 240,000pps, mid-70 100,000
                                    elif dataType == 2:  # Cartesian/single return/N=96
                                        # to account for first point's timestamp being increment in the loop
                            ...
                            if select.select([self.i_socket], [], [], 0)[0]:
                                ...
                                # Horizon, avia and Tele-15 IMU data packet
                                if dataType == 6:
                                    if not IMU_reporting:
                                        ...
                                        # full_name = os.path.join(path_file.parent, filename + "_IMU" + exten)
                                        IMU_file = open(os.path.join(path_file.parent, filename + "_IMU" + exten), "wb")
...
class openpylivox(object):
    ...
    _CMD_LIDAR_DUAL =             bytes.fromhex((b'AA011000000000B8090106022DD4AEAD').decode('ascii'))  # little
    _CMD_LIDAR_TRIPLE =           bytes.fromhex((b'AA011000000000B8090106031871cd44').decode('ascii'))  # not sure the crc16/32 checksum
    ...
    def _bindPorts(self):
        ...
        try:
            self._dataSocket.bind((self._computerIP, self._dataPort))  # 数据
            self._cmdSocket.bind((self._computerIP, self._cmdPort))  # 命令/配置
            self._imuSocket.bind((self._computerIP, self._imuPort))  # imu
    ...
    def _parseResp(self, binData):
        ...
        # aa = binascii.hexlify(dataBytes[8])
        # bb = binascii.hexlify(dataBytes[7])
    ...
    def _crc16(self, data):
        # http://crcmod.sourceforge.net/crcmod.html?highlight=mkcrcfun
        # @ ploy:  The generator polynomial to use in calculating the CRC.
        crc16 = crcmod.mkCrcFun(0x11021, rev=True, initCrc=0x4C49)  # 0x11021 == x^16+x^12+x^5+x^0
        checkSum = crc16(data)
        return checkSum
    ...
    def _crc32(self, data):
        # 0x104C11DB7 == +x^32+x^26+x^23+x^22+x^16+x^12+x^11+x^10+x^8+x^7+x^5+x^4+x^2+x^1+x^0
    ...
    def _dataStart_RT_B(self):
        ...
        if self._isConnected:
            if not self._isData:
                # self._captureStream = _dataCaptureThread(self._sensorIP, self._dataSocket, self._imuSocket, "", 1, 0, 0, 0, self._showMessages, self._format_spaces, self._deviceType)
                ...
                time.sleep(0.12)  # 0.12 second for what?
                self._waitForIdle()
                self._cmdSocket.sendto(self._CMD_DATA_START, (self._sensorIP, 65000))  # 发送控制指令，开始接受，到目标设备IP和端口
                if self._showMessages: print("   " + self._sensorIP + self._format_spaces + "   <--     sent start data stream request")

                # check for proper response from data start request
                if select.select([self._cmdSocket], [], [], 0.1)[0]:  # ACK
                    binData, addr = self._cmdSocket.recvfrom(16)  # 接收头 binData 和 ip
                    _, ack, cmd_set, cmd_id, ret_code_bin = self._parseResp(binData)  # 数据验证，
                    # cmd_id = 4: Start/Stop Sampling
                    if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "4":
                        ret_code = int.from_bytes(ret_code_bin[0], byteorder='little')
                        if ret_code == 1:
                            if self._showMessages: print("   " + self._sensorIP + self._format_spaces + "   -->     FAILED to start data stream")
    ...
    # @ path @ wait time (s) @ collect time (s)
    def _saveDataToFile(self, filePathAndName, secsToWait, duration):

        if self._isConnected:
            if self._isData:
                if self._firmware != "UNKNOWN":  # known lidar type
                    try:
                            ...
                            if secsToWait < 0:
                                ...
                                if secsToWait > 900:
                                    ...
                                    if filePathAndName == "":
                                        ...
                                        # successful bin data processing<-----
                                        # only change state variable
    ...
    def saveDataToFile(self, filePathAndName, secsToWait, duration):
        ...
        filename = path_file.stem
        exten = path_file.suffix
        self._saveDataToFile(filePathAndName, secsToWait, duration)  # core fun
        for i in range(len(self._mid100_sensors)):  # 每个该函数用于处理mid-100内的三个mid-40
            new_file = ""
            if i == 0:
    ...
    def doneCapturing(self):
        stop = []
        stop.append(self._doneCapturing())  # getting data??
        for i in range(len(self._mid100_sensors)):
    ...
    try:
        ...
        if os.path.exists(filePathAndName) and os.path.isfile(filePathAndName):
            ...
            if checkMessage == "OPENPYLIVOX":
                ...
                filename = path_file.stem
                exten = path_file.suffix
                IMU_file = os.path.join(path_file.parent, filename + "_IMU" + exten)
...
def convertBin2CSV(filePathAndName, deleteBin=False):
    ...
    # mid-100 contains 3 mid-40, left, mid, right
...
def data_table(firmwareType, dataType):
    if firmwareType == 1 and dataType == 0:
        dataClass = 1
        divisor = 21
    elif firmwareType == 1 and dataType == 1:
        ...
        dataClass = 8
        divisor = 24
    return dataClass, divisor


def convertBin2PCD(path):

    if os.path.exists(path):
        filelist = os.listdir(path)
        for file in filelist:
            if not '_IMU' in file and '.bin' in file:
                print(file)
                points = []
                rgb = []
                file_path = os.path.join(path, file)
                # bin_size = Path(file_path).stat().st_size - 15
                ...
                if checkMessage == "OPENPYLIVOX":
                    ...
                    if firmwareType >= 1 and firmwareType <= 3:
                        if dataType >= 0 and dataType <= 5:
                            print("CONVERTING OPL BINARY DATA, PLEASE WAIT...")
                            dataClass, divisor = data_table(firmwareType, dataType)
                            #print(dataClass, divisor)
                            while True:
                                try:
                                    if dataClass == 5:
                                        coord1 = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord2 = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord3 = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        intensity = float(struct.unpack('<B', binFile.read(1))[0]) / 255.0
                                        tag_bits = str(bin(int.from_bytes(binFile.read(1), byteorder='little')))[2:].zfill(8)
                                        spatial_conf = str(int(tag_bits[0:2], 2))
                                        intensity_conf = str(int(tag_bits[2:4], 2))
                                        returnType = str(int(tag_bits[4:6], 2))
                                        timestamp_sec = float(struct.unpack('<d', binFile.read(8))[0])
                                        points.append([coord1, coord2, coord3])
                                        rgb.append([intensity, intensity, intensity])
                                    else:
                                        break
                                except:
                                    break
                ...
                if points and rgb:
                    ...
                    o3d.io.write_point_cloud(file_path.replace('.bin', '.pcd'), pcd, write_ascii=True, compressed=True)
                os.remove(file_path)
            elif "_IMU" in file:
                print(file)
